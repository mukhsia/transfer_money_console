    ...
    internal class Person
    {
        ...
        public bool TransferMoney(Person receiver, int amountToTransfer)
        {
            ...
            if(amountToTransfer <= this.wallet.money)
            {
                ...
                Console.WriteLine($"{this.name} transferred ${amountToTransfer} to {receiver.name}.");
                return true;
            } 
            else
            {
                Console.WriteLine($"{this.name} has insufficient funds. Attempts to transfer ${amountToTransfer} with ${this.wallet.money} balance.");
                return false;
            }
        }
